...
int FCB_reserve(size_t num, Fid_t *fid, FCB** fcb)
{
	...
	/* Find distinct fids */
	/* Loop through the current process' FIDs and try to find num FIDs */
	for(i=0; i<num; i++){
		while(f<MAX_FILEID && cur->FIDT[f]!=NULL)
			f++;
		/* Did not find any FID avaiable */
		if(f==MAX_FILEID) 
			break;
		fid[i] = f; 
		f++;
	}
	/* Did not find num FIDs return 0 */
	if(i<num) 
		return 0;
	/* Allocate FCBs */
	/* Try to allocate num FCBs */
	for(i=0;i<num;i++)
		if((fcb[i] = acquire_FCB()) == NULL)
			break;
	/* If unable to allocate num FCBs roll back any changes and return 0 */
	if(i<num){
		/* Roll back */
		while(i>0){
			release_FCB(fcb[i-1]);
